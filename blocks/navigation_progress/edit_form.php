...
/**
 * Navigation Progress block configuration form definition
 *
...
/**
 * Navigation Progress block config form class
 *
 * @copyright  2021 Nikolay Terekhin
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_navigation_progress_edit_form extends block_edit_form {
    /**
    ...
    protected function specific_definition($mform) {
        ...
        $activities = block_navigation_progress_get_activities($COURSE->id, null, 'orderbycourse');
        ...
        $options = array(
            'orderbytime'   => get_string('config_orderby_due_time', 'block_navigation_progress', $expectedbystring),
            'orderbycourse' => get_string('config_orderby_course_order', 'block_navigation_progress'),
        );
        $label = get_string('config_orderby', 'block_navigation_progress');
        $mform->addElement('select', 'config_orderby', $label, $options);
        $mform->setDefault('config_orderby', DEFAULT_COMPLETIONPROGRESS_ORDERBY);
        $mform->addHelpButton('config_orderby', 'how_ordering_works', 'block_navigation_progress');
        ...
        if (!$allwithexpected) {
            $warningstring = get_string('not_all_expected_set', 'block_navigation_progress', $expectedbystring);
        ...
        $label = get_string('config_longbars', 'block_navigation_progress');
        ...
        $mform->addHelpButton('config_longbars', 'how_longbars_works', 'block_navigation_progress');
        ...
        if (get_config('block_navigation_progress', 'forceiconsinbar') !== "1") {
            $mform->addElement('selectyesno', 'config_progressBarIcons',
                               get_string('config_icons', 'block_navigation_progress').' '.
                               $OUTPUT->pix_icon('tick', '', 'block_navigation_progress', array('class' => 'iconOnConfig')).
                               $OUTPUT->pix_icon('cross', '', 'block_navigation_progress', array('class' => 'iconOnConfig')));
            $mform->setDefault('config_progressBarIcons', DEFAULT_COMPLETIONPROGRESS_PROGRESSBARICONS);
            $mform->addHelpButton('config_progressBarIcons', 'why_use_icons', 'block_navigation_progress');
        }

        // Allow progress percentage to be turned on for students.
        $mform->addElement('selectyesno', 'config_showpercentage',
                           get_string('config_percentage', 'block_navigation_progress'));
        $mform->setDefault('config_showpercentage', DEFAULT_COMPLETIONPROGRESS_SHOWPERCENTAGE);
        $mform->addHelpButton('config_showpercentage', 'why_show_precentage', 'block_navigation_progress');
        ...
        if (!empty($groups) || !empty($groupings)) {
            ...
            $label = get_string('config_group', 'block_navigation_progress');
            $mform->addElement('select', 'config_group', $label, $options);
            $mform->setDefault('config_group', '0');
            $mform->addHelpButton('config_group', 'how_group_works', 'block_navigation_progress');
            $mform->setAdvanced('config_group', true);
        }

        // Set block instance title.
        $mform->addElement('text', 'config_progressTitle',
                           get_string('config_title', 'block_navigation_progress'));
        $mform->setDefault('config_progressTitle', '');
        $mform->setType('config_progressTitle', PARAM_TEXT);
        $mform->addHelpButton('config_progressTitle', 'why_set_the_title', 'block_navigation_progress');
        $mform->setAdvanced('config_progressTitle', true);

        // Control which activities are included in the bar.
        $options = array(
            'activitycompletion' => get_string('config_activitycompletion', 'block_navigation_progress'),
            'selectedactivities' => get_string('config_selectedactivities', 'block_navigation_progress'),
        );
        $label = get_string('config_activitiesincluded', 'block_navigation_progress');
        ...
        $mform->addHelpButton('config_activitiesincluded', 'how_activitiesincluded_works', 'block_navigation_progress');
        $mform->setAdvanced('config_activitiesincluded', true);

        // Check that there are activities to monitor.
        if (empty($activities)) {
            $warningstring = get_string('no_activities_config_message', 'block_navigation_progress');
            ...
            $label = get_string('config_selectactivities', 'block_navigation_progress');
            ...
            $mform->addHelpButton('config_selectactivities', 'how_selectactivities_works', 'block_navigation_progress');
        }
    }
}
