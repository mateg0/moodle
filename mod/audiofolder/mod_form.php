...
/**
 * audiofolder configuration form
 *
...
class mod_audiofolder_mod_form extends moodleform_mod {
    function definition() {
        global $CFG;
        $mform = $this->_form;

        $config = get_config('audiofolder');
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'content', get_string('contentheader', 'audiofolder'));
        $mform->addElement('filemanager', 'files', get_string('files'), null, array('subdirs'=>1, 'accepted_types'=>['.mp3', '.pcm', '.wav', '.aac', '.ogg', '.wma']));
        $mform->addElement('select', 'display', get_string('display', 'mod_audiofolder'),
                array(audiofolder_DISPLAY_PAGE => get_string('displaypage', 'mod_audiofolder'),
                    audiofolder_DISPLAY_INLINE => get_string('displayinline', 'mod_audiofolder')));
        $mform->addHelpButton('display', 'display', 'mod_audiofolder');
        if (!$this->courseformat->has_view_page()) {
            $mform->setConstant('display', audiofolder_DISPLAY_PAGE);
    ...
    function data_preprocessing(&$default_values) {
        if ($this->current->instance) {
            ...
            file_prepare_draft_area($draftitemid, $this->context->id, 'mod_audiofolder', 'content', 0, array('subdirs'=>true));
    ...
    function validation($data, $files) {
        ...
        if (empty($errors['completion']) &&
                ...
                $data['display'] == audiofolder_DISPLAY_INLINE) {
            $errors['completion'] = get_string('noautocompletioninline', 'mod_audiofolder');
        }
