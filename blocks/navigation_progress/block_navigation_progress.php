...
/**
 * Navigation Progress block definition
 *
...
/**
 * Navigation Progress block class
 *
 * @copyright  2021 Nikolay Terekhin
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_navigation_progress extends block_base {
    ...
    public function init() {
        $this->title = get_string('pluginname', 'block_navigation_progress');
    }
...
/*
            ...
            if (($this->page->user_is_editing() || is_siteadmin()) && empty($courses)) {
                $this->content->text = get_string('no_courses', 'block_navigation_progress');
                return $this->content;
            }

            $coursenametoshow = get_config('block_navigation_progress', 'coursenametoshow') ?:
            ...
            $sql = "SELECT bi.id,
                 ...
                 LEFT JOIN {block_positions} bp ON bp.blockinstanceid = bi.id
                     ...
                     WHERE bi.blockname = 'navigation_progress'
            ...
            foreach ($courses as $course) {
                ...
                if ($course->visible && $completion->is_enabled()) {
                    ...
                    foreach ($blockinstances as $blockid => $blockinstance) {
                        ...
                        $blockinstance->activities = block_navigation_progress_get_activities($course->id, $blockinstance->config);
                        $blockinstance->activities = block_navigation_progress_filter_visibility($blockinstance->activities,
                                                         $USER->id, $course->id, $exclusions);
//                        echo'<pre>';
//                        var_dump($blockinstance->activities);
//                        echo '</pre>';

                        $blockcontext = CONTEXT_BLOCK::instance($blockid);
                        if (
                            !has_capability('block/navigation_progress:showbar', $blockcontext) ||
                            $blockinstance->visible == 0 ||
                            empty($blockinstance->activities) ||
                            (
                                !empty($blockinstance->config->group) &&
                                !has_capability('moodle/site:accessallgroups', $context) &&
                                !block_navigation_progress_group_membership($blockinstance->config->group, $course->id, $USER->id)
                            )
                        ) {
                    ...
                    foreach ($blockinstances as $blockid => $blockinstance) {
                        ...
                        $completions = block_navigation_progress_completions($blockinstance->activities, $USER->id, $course,
                            $submissions);
                        $this->content->text .= block_navigation_progress_bar($blockinstance->activities,
                                                                    $completions,
...
//            echo '</pre>';
            ...
            if (empty($activities)) {
                if (has_capability('moodle/block:edit', $this->context)) {
                    $this->content->text .= get_string('no_activities_config_message', 'block_navigation_progress');
                }
                return $this->content;
            }
...
//        }
        ...
        $this->page->requires->css('/blocks/navigation_progress/css.php?v=' . $cachevalue);
    ...
    public static function increment_cache_value() {
        $value = get_config('block_navigation_progress', 'cachevalue') + 1;
        set_config('cachevalue', $value, 'block_navigation_progress');
    }
}
