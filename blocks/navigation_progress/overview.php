...
/**
 * Navigation Progress block overview page
 *
...
$PAGE->set_url(
    '/blocks/navigation_progress/overview.php',
    array(
        'instanceid' => $id,
        'courseid'   => $courseid,
        'page'       => $page,
...
$title = get_string('overview', 'block_navigation_progress');
$PAGE->set_title($title);
$PAGE->set_heading($title);
$PAGE->navbar->add($title);
$PAGE->set_pagelayout('report');

$cachevalue = debugging() ? -1 : (int)get_config('block_navigation_progress', 'cachevalue');
$PAGE->requires->css('/blocks/navigation_progress/css.php?v=' . $cachevalue);

// Check user is logged in and capable of accessing the Overview.
require_login($course, false);
require_capability('block/navigation_progress:overview', $blockcontext);
confirm_sesskey();

$output = $PAGE->get_renderer('block_navigation_progress');

// Start page output.
echo $OUTPUT->header();
echo $OUTPUT->heading($title, 2);
echo $OUTPUT->container_start('block_navigation_progress');

// Check if activities/resources have been selected in config.
$activities = block_navigation_progress_get_activities($courseid, $config);
if ($activities == null) {
    echo get_string('no_activities_message', 'block_navigation_progress');
    echo $OUTPUT->container_end();
    echo $OUTPUT->footer();
    die();
}
if (empty($activities)) {
    echo get_string('no_visible_activities_message', 'block_navigation_progress');
    echo $OUTPUT->container_end();
    echo $OUTPUT->footer();
    die();
}
...
if (get_config('block_navigation_progress', 'showinactive') !== "1") {
...
$submissions = block_navigation_progress_submissions($course->id);
...
$formattributes['data-table-unique-id'] = 'block-navigation_progress-overview-' . $course->id;
...
if (get_config('block_navigation_progress', 'showlastincourse') !== "0") {
    $tablecolumns = array('fullname', 'lastonline', 'progressbar', 'progress');
    $tableheaders = array(
                        get_string('fullname'),
                        get_string('lastonline', 'block_navigation_progress'),
                        get_string('progressbar', 'block_navigation_progress'),
                        get_string('progress', 'block_navigation_progress')
                    );
} else {
...
if ($sortbyprogress) {
    ...
    usort($users, 'block_navigation_progress_compare_rows');
...
$rows = array();
$exclusions = block_navigation_progress_exclusions($course->id);
for ($i = $startuser; $i < $enduser; $i++) {
    ...
    $useractivities = block_navigation_progress_filter_visibility($activities, $users[$i]->id, $course->id, $exclusions);
    if (!empty($useractivities)) {
        $completions = block_navigation_progress_completions($useractivities, $users[$i]->id, $course, $users[$i]->submissions);
        $progressbar = block_navigation_progress_bar($useractivities, $completions, $config, $users[$i]->id, $course->id,
            $block->id, true);
        $progressvalue = block_navigation_progress_percentage($useractivities, $completions);
        $progress = $progressvalue.'%';
    } else {
        $progressbar = get_string('no_visible_activities_message', 'block_navigation_progress');
        $progressvalue = 0;
        $progress = '?';
    }
...
// Sort the user rows.
if ($sortbyprogress) {
    usort($rows, 'block_navigation_progress_compare_rows');
}
    ...
    for ($i = $startdisplay; $i < $enddisplay; $i++) {
        ...
        if ($bulkoperations) {
            $checkbox = new core\output\checkbox_toggleall('participants-table', false, [
                ...
                'checked' => false,
                'label' => get_string('selectitem', 'block_navigation_progress', $rows[$i]['fullnametext']),
...
if ($bulkoperations) {
    ...
    $PAGE->requires->js_call_amd('block_navigation_progress/overview', 'init', [$options]);
}
...
$PAGE->requires->js_call_amd('block_navigation_progress/progressbar', 'init', [
 ...
 */
function block_navigation_progress_compare_rows($a, $b) {
    global $sort;
