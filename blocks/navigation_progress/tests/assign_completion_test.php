...
/**
 * Assignment activity-related unit tests for Navigation Progress block.
 *
...
class assign_completion_testcase extends \advanced_testcase {
    ...
    private function assert_progress_completion($course, $student, $cm, $status) {
        $activities = [ ['id' => $cm->id ]];
        $submissions = block_navigation_progress_submissions($course->id, $student->id);
        $completions = block_navigation_progress_completions($activities, $student->id,
            $course, $submissions);
        $this->assertEquals(
            [$cm->id => $status],
            $completions
