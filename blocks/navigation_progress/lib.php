...
/**
 * Navigation Progress block common configuration and helper functions
 *
 ...
 */
const DEFAULT_COMPLETIONPROGRESS_WRAPAFTER = 8;
 ...
 */
function block_navigation_progress_submissions($courseid, $userid = 0) {
    global $DB, $CFG;
 ...
 */
function block_navigation_progress_modules_with_alternate_links() {
    global $CFG;
 ...
 */
function block_navigation_progress_get_activities($courseid, $config = null, $forceorder = null) {
    ...
    if ($forceorder == 'orderbycourse' || ($config && $config->orderby == 'orderbycourse')) {
        usort($activities, 'block_navigation_progress_compare_events');
    } else {
        usort($activities, 'block_navigation_progress_compare_times');
    }
 ...
 */
function block_navigation_progress_compare_events($a, $b) {
 ...
 */
function block_navigation_progress_compare_times($a, $b) {
    if (
        $a['expected'] != 0 &&
        $b['expected'] != 0 &&
        ...
        return block_navigation_progress_compare_events($a, $b);
    }
}
 ...
 */
function block_navigation_progress_filter_visibility($activities, $userid, $courseid, $exclusions) {
    global $CFG;
 ...
 */
function block_navigation_progress_completions($activities, $userid, $course, $submissions) {
    $completions = array();
 ...
 */
function block_navigation_progress_bar($activities, $completions, $config, $userid, $courseid, $instance, $simple = false) {
    ...
    $alternatelinks = block_navigation_progress_modules_with_alternate_links();

    // Get relevant block instance settings or use defaults.
    if (get_config('block_navigation_progress', 'forceiconsinbar') !== "1") {
    ...
    $defaultlongbars = get_config('block_navigation_progress', 'defaultlongbars') ?: DEFAULT_COMPLETIONPROGRESS_LONGBARS;
...
//    $barclasses = array('barRow');
    $barclasses = array('course-progress');
    $content .= html_writer::start_div('barContainer', ['data-instanceid' => $instance]);

    // Determine the segment width.
    $wrapafter = get_config('block_navigation_progress', 'wrapafter') ?: DEFAULT_COMPLETIONPROGRESS_WRAPAFTER;
    if ($wrapafter <= 1) {
        $wrapafter = 1;
    }
    ...
    if ($orderby == 'orderbytime' && $longbars != 'wrap' && $displaynow == 1 && !$simple) {
        ...
        $nowstring = get_string('now_indicator', 'block_navigation_progress');
        $leftarrowimg = $OUTPUT->pix_icon('left', $nowstring, 'block_navigation_progress', array('class' => 'nowicon'));
        $rightarrowimg = $OUTPUT->pix_icon('right', $nowstring, 'block_navigation_progress', array('class' => 'nowicon'));
    }
...
//    echo '</pre>';
    ...
    $sectionsArr = array();
    foreach($activities as $activity) {
        if (!isset($sectionsArr[$activity['section']])) {
            $sectionsArr[$activity['section']] = array($activity);
        }
        else {
            array_push($sectionsArr[$activity['section']], $activity);
        }
    }
    ksort($sectionsArr);
    ...
    foreach ($sectionsArr as $sectionId => $section) {
        ...
        // Start progress bar.
        $rowoptions['id'] = 'sectionBar'.$instance.'-'.$userid.'-'.$sectionId;
...
//    $content .= html_writer::start_div('barRowCells', $cellsoptions);
        $content .= html_writer::start_div('bar', $cellsoptions);
        $counter = 1;
        ...
        foreach ($section as $activity) {
            ...
            $cellcontent = '';
            $celloptions = array(
//                'class' => ' activity progressBarCell',
                'class' => '',
...
//        $content .= HTML_WRITER::div($cellcontent, null, $celloptions);
    ...
    if ($showpercentage == 1 && !$simple) {
        $progress = block_navigation_progress_percentage($activities, $completions);
        $percentagecontent = get_string('progress', 'block_navigation_progress').': '.$progress.'%';
    ...
    if (!$simple) {
        $content .= get_string('mouse_over_prompt', 'block_navigation_progress');
        $content .= ' ';
        $attributes = array (
            'class' => 'accesshide progressShowAllInfo',
        );
        $content .= HTML_WRITER::link('#', get_string('showallinfo', 'block_navigation_progress'), $attributes);
    }
    ...
    foreach ($activities as $activity) {
        ...
        $content .= $OUTPUT->pix_icon($icon, $altattribute, 'block_navigation_progress', array('class' => 'iconInInfo'));
        $content .= HTML_WRITER::empty_tag('br');
        if ($activity['expected'] != 0) {
            $content .= HTML_WRITER::start_tag('div', array('class' => 'expectedBy'));
            $content .= get_string('time_expected', 'block_navigation_progress').': ';
 ...
 */
function block_navigation_progress_percentage($activities, $completions) {
    $completecount = 0;
 ...
 */
function block_navigation_progress_exclusions ($courseid, $userid = null) {
    global $DB;
 ...
 */
function block_navigation_progress_group_membership ($group, $courseid, $userid) {
    if ($group === '0') {
        return true;
