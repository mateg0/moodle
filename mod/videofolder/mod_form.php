...
/**
 * videofolder configuration form
 *
...
class mod_videofolder_mod_form extends moodleform_mod {
    function definition() {
        global $CFG;
        $mform = $this->_form;

        $config = get_config('videofolder');
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'content', get_string('contentheader', 'videofolder'));
        $mform->addElement('filemanager', 'files', get_string('files'), null, array('subdirs'=>1, 'accepted_types'=>['.mp4', '.asf', '.avi', '.mkv', '.mov', '.vob', '.wmv', '.webm']));
        $mform->addElement('select', 'display', get_string('display', 'mod_videofolder'),
                array(videofolder_DISPLAY_PAGE => get_string('displaypage', 'mod_videofolder'),
                    videofolder_DISPLAY_INLINE => get_string('displayinline', 'mod_videofolder')));
        $mform->addHelpButton('display', 'display', 'mod_videofolder');
        if (!$this->courseformat->has_view_page()) {
            $mform->setConstant('display', videofolder_DISPLAY_PAGE);
    ...
    function data_preprocessing(&$default_values) {
        if ($this->current->instance) {
            ...
            file_prepare_draft_area($draftitemid, $this->context->id, 'mod_videofolder', 'content', 0, array('subdirs'=>true));
    ...
    function validation($data, $files) {
        ...
        if (empty($errors['completion']) &&
                ...
                $data['display'] == videofolder_DISPLAY_INLINE) {
            $errors['completion'] = get_string('noautocompletioninline', 'mod_videofolder');
        }
